...
export class CreateShowtimeComponent implements OnInit {
    constructor(private movies: MoviesAdminService, private cinemas: CinemaService,private showtime: CreateShowTimeAdminService, private rooms: RoomService,  private shifts: ShiftService) {}
    ngOnInit(): void {
    ...
    addShowtime(){
      const newShowTime: ShowTimedto = new ShowTimedto();
        ...
        this.showtime.addShowTime(newShowTime).subscribe((res) => {
            console.log('push')
            this.showTimeDto.push(res);
